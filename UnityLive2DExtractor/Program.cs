...
namespace UnityLive2DExtractor
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var tmpBasePathList = cubismMocs
                .Select(cubismMoc => containers[cubismMoc]) // 获取文件路径字符串
                .Select(container =>
                {
                    int lastSlashIndex = container.LastIndexOf('/');
                    int secondLastSlashIndex = container.LastIndexOf('/', lastSlashIndex - 1);
                    return container.Substring(0, secondLastSlashIndex);
                }) // 提取再上一级父文件夹路径
                .ToList();
            var tmpLookup = containers.ToLookup(x => tmpBasePathList.Find(b => x.Value.Contains(b)) + "/mtn", x => x.Key);

            var basePathList = cubismMocs
                .Select(cubismMoc => containers[cubismMoc]) // 获取文件路径字符串
                .Select(container => container.Substring(0, container.LastIndexOf('/'))) // 提取父文件夹路径
                .ToList();
            ...
            foreach (var assets in lookup)
            {
                ...
                var motionsDirectory = key.Substring(0, key.LastIndexOf('/')) + "/mtn";
                foreach (var asset in tmpLookup[motionsDirectory])
                {
                    if (asset is AnimationClip m_AnimationClip)
                    {
